...
5. **DPI 感知只能运行时设，manifest 声明会 14001 拒启**（2026-09-10）：宿主不跑 winit，缺声明时系统按 96 DPI 虚拟化再拉伸 1.25x → 整窗文字发糊；往 winresource 的 manifest 里加 `<application><windowsSettings><dpiAwareness>` 段后 exe 直接起不来（「并行配置不正确」os error 14001，rc/SXS 对那段命名空间组合不认）。正路 = `host_main()` 开头 `SetProcessDpiAwarenessContext(PMv2)`（winit 同款），失败退 `SetProcessDPIAware()`；建窗尺寸 1200×800 逻辑值自己乘 DPI；`WM_DPICHANGED` 按建议矩形跟缩。验证：`GetDpiForWindow(hwnd)` 应为 120（125% 屏）。
6. **手柄环透明的设计是真的**：`WM_ERASEBKGND` 返回 1 = 环带 alpha=0 透出桌面。别「修」成刷色——2026-09-09 用户截图白圈其实是桌面本来就白，刷成 RING_BG 反而把透明外圈毁了，已回滚。
7. **运行时检测**：`reg query` EdgeUpdate 客户端 GUID `{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}`（HKLM WOW6432/HKLM/HKCU 任一存在即在）。缺则 `webview::open` 返回 Err、toast 提示安装，**按用户要求不静默回退浏览器**。
8. 依赖：`wry` + `windows`（Foundation/WindowsAndMessaging/Gdi/LibraryLoader/Com）+ `raw-window-handle`。exe 体积 +~0.3MB（不打包内核，WebView2Loader 系统自带动态加载）。
