...
侧边栏点一下就整块内容瞬间换掉，看不出发生了什么。加的过渡只有两个量，都由同一个补间 `anim::PAGE` 驱动（**1 = 刚切过来、0 = 已落定**，方向刻意反着：`AnimState::value` 对无记录的 key 返回 0.0，正好是落定态，于是冷启动、`--page` 出图都不必预置初值）：

| 量 | 做法 | 为什么不用别的 |
|---|---|---|
| 内容整体下移 14px → 0 | `renderer.with_translation` 包住 `draw` | **不动 padding/height**：那会让每帧重新布局，设置页六张卡要整树重排；平移只影响 draw，布局结果逐帧复用 |
| 背景色面纱 0.9α → 0 | 内容之上盖一层 `bg_app` 半透明 quad | iced 0.14 的 `renderer::Style` 只有 `text_color`，**没有全局 opacity**，wgpu 后端也没有「离屏渲染一层再整体调 alpha」的入口。模态遮罩用的就是这招 |

两个必须记住的细节：

1. **面纱要自己开一层**（`renderer.with_layer`）。两个后端在同一层里都是「先画所有 quad，再画所有 text」（wgpu 见 `iced_wgpu/src/lib.rs` 的 render 循环，tiny-skia 同序），面纱若落在内容那一层，文字会盖在它上面——底色淡入而文字全程清晰，比不做动画更怪。
2. **不用 `Transformation::scale`**。缩放会连文字一起缩，cosmic-text 在非整数缩放下要么每帧重新栅格化（白扔性能）要么拉伸图集（更糊）。字本来就没有 hinting，不能再糊。

`Reveal` 在 widget 树里是**透明的**——tag/state/children/layout/update 全部直通内容（抄 `iced_widget::themer`），所以补间值变化不引起任何树 diff；`t == 0` 时直接走原路，不推变换也不画面纱，落定后零开销。
...
取证方式：`--switch-to <页> --switch-at <毫秒>` 配 `--shot ... --after`，两者的差就是快门落在过渡的第几毫秒（单靠 `--after` 永远只截得到落定态）。实测 t≈0.75 时主区平均亮度 9.8、t≈0.35 时 23.5、落定 25.5，内容位移 3px 未落定——面纱与平移都在动。空闲零仍成立：`--drawlog` 显示切页那 5s 出 31 帧，之后每 5s `delta=0`。
